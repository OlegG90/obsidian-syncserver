 ...
 * device row is. That is two places to remember when the answer changes, and D118 had just changed it:
 ...
 *
 * No keys and no cursors, on either screen. An administrator holds nothing that opens a vault (#115), and
 * a device row is not where that would start.
 */
import type { Db } from './db.js';

/**
 * What a device row carries. `lastSeenAt` is as fresh as the access token's lifetime — see D118.
 *
