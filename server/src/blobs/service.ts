...
export class BlobService {
  ...
  private async recordUpload(
    ...
    // The schema guards the blob and its claim with triggers; `txGuarded` turns one of those into a
    // refusal the caller can act on, rather than a 500 — the rule the node writes already follow.
    return txGuarded(this.db, async (c) => {
