 ...
 *
 * **The column is written when a device refreshes its session, not when it syncs** (D-118),
 * so its freshness is the access token's lifetime — the server says which, and this says it
 * in the units of that bound. A precise timestamp claimed an exactness the value does not
 * have: two devices reading eleven minutes behind a vault that was visibly busy looked like
 * two devices that had stopped talking, which is how a diagnosis went wrong on a live server.
 *
 * The exact instant is not lost; it belongs on the row's `title`, where somebody who wants it
 * can ask for it.
 *
 * The plugin's own device list says the same thing in the same words — `plugin/src/seen.ts`.
 * Two bundles, no runtime code between them (`shared` is declarations only, deliberately), so
 * the wording is kept in step by being written the same and tested on both sides.
 */
