...
# No schema comparison here any more (D-132). It used to compare function and trigger names and
# exit when the database was behind; the server now applies its own migrations at start and exits
# when it cannot, so a database the build does not match shows up below as a server that never
# becomes healthy, with the reason in `docker compose logs server`.
