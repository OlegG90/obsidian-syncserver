...
| Devices | shown under the vault each one syncs (D-139), with last seen **as an interval** — the column moves on a session renewal rather than on a sync (D-118), so the screen states the bound the server sends and keeps the exact instant on hover (#386) — **rename any of them**, **sign out this device**, sign out everywhere. The operator's view of an account's devices groups them by vault id — the console cannot read vault names (D-115) — and can rename one too, recorded as `device.rename` with both names (D-133); the console's own device is not renameable, since each sign-in names it |
...
`server/src/backup.ts` (`verifyBackup`).
