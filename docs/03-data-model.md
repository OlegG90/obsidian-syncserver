...
| `backup_runs` | `id`, `started_at`, `writes_frozen_at`, `writes_thawed_at`, `blobs_done_at`, `db_done_at`, `finished_at`, `status`, `bytes`, `blob_count`, `destination`, `error`, `verified_at`, `triggered_by` | backup history. A run records the **freeze window** and both legs, and `CHECK`s reject any run whose leg falls outside it (#95) — the procedure is enforced rather than described. A `failed` run must carry its reason; `verified_at` is the last successful restore drill |
...
| the **first administrator** — `users`, `role = 'admin'`, `state = 'provisioned'` | every account is born from an invitation an administrator issues (#83), so the first one has nobody to invite it. The server cannot make a finished account either: the keys are born on a device. An unredeemed invitation is the only shape it can hold |

The bootstrap invitation's token is the literal string `admin` — a default credential, made safe by being
**single use by construction**: redeeming it is what replaces it, filling in the operator's own key material
and turning that row into their account. The application refuses to serve anything but that redemption until
it happens ([04](04-sync-protocol.md)), so the exposure is the first run rather than for ever.
