...
| `devices` | `id`, `user_id`, `name`, `platform`, `last_seen_at`, `refresh_token_hash`, `revoked_at`, `vault_id` | a device is **a vault connected on a machine** (D-139, revising AC-13): `vault_id` names the vault it syncs, set at redeem or learned when it first opens one, never moved afterwards, and cleared when that vault is removed — which also revokes the device. Its token still reaches every vault of the account. Cursors are client-owned and per vault, so the device table stores no authoritative cursor (only a diagnostic `last_cursor` the client may sync). One refresh token **per device**, so signing out one device is possible at all (D-90); `refresh_token_hash` is SHA-256 hex like the other three verifiers (D-108) |
