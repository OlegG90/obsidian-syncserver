...
| a full rescan ☑ | changes made outside Obsidian, rename detection by hash. A pass **walks every path** and compares against what it recorded, so a change no event announced is still found — vault events decide *when* a pass runs (issue #238), never what it looks at. What a pass no longer does is *read* every file: a path whose `mtime` and `size` are unchanged keeps its recorded hash (issue #237, docs/04), and the **Full rescan** command reads everything for the case a timestamp lied |
