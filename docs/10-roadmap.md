...
| a full rescan ☑ | changes made outside Obsidian, rename detection by hash. A pass **walks every path** and compares against what it recorded, so a change no event announced is still found — vault events decide *when* a pass runs (issue #238), never what it looks at. It **reads** the files it cannot rule out: a path whose `mtime` and `size` match what was recorded keeps its stored hash (issue #237, docs/04). **Full rescan** reads everything, for the case a timestamp lied |
