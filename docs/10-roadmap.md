...
| a full rescan ☑ | changes made outside Obsidian, rename detection by hash. A sync walks every path (there is no watcher) but no longer *reads* every file — a path whose `mtime` and `size` are unchanged keeps its recorded hash (issue #237, docs/04). The reading is what the **Full rescan** command restores, for the case a timestamp lied |
