...
| 95 | ~~A backup is one frozen window, not an ordering~~ — **superseded by #114**, which keeps the window and reverses the second half: the order *is* normative, because this server refuses new writes rather than stopping running ones. What survives is the record: `backup_runs` holds the window and `CHECK`s reject a leg outside it — plus one #95 did not have, that the database leg finished before the blob leg |
